...
%            fStartTime(f) = datenum(datetime('now'));
            ...
            % Save qdotImage contourImage xSize and ySize and tSize
            save(fullfile(xFolder,['contourImage ' fFilename, '.mat']),'contourImage')
            save(fullfile(xFolder,['qdotImage ' fFilename, '.mat']),'qdotImage')
            save(fullfile(xFolder,['xSize ' fFilename, '.mat']),'xSize')
            save(fullfile(xFolder,['ySize ' fFilename, '.mat']),'ySize')
            save(fullfile(xFolder,['tSize ' fFilename, '.mat']),'tSize')
            ...
            % Save qdotCropped
            save(fullfile(xFolder,['qdotNormaized ' fFilename, '.mat']),'contourNormalized')
            ...
            % Save the contour, and contourExpanded
            save(fullfile(xFolder, ['ContourExpanded ' fFilename,'.mat']),'contourExpanded')
            save(fullfile(xFolder,['contourImage ' fFilename, '.mat']),'contourImage')
            
            % Reshape contact index to matrix and save
            %reshape(cellContacts(tt).PixelIdxList, [xSize, ySize])
            %reshape(cellContacts(tt).PixelIdxList, [ySize, xSize])
